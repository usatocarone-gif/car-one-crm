...
type ChannelSellerSummary = { name: string; leadQuotes: number; leadContracts: number; directQuotes: number; directContracts: number };
...
function summarizeChannelSellers(leadItems: ChannelCohortItem[], directItems: ChannelCohortItem[]) {
  ...
  const add = (items: ChannelCohortItem[], kind: "lead" | "direct") => items.forEach((item) => (item.sellers ?? []).forEach((seller) => {
    const current = grouped.get(seller.name) ?? { name: seller.name, leadQuotes: 0, leadContracts: 0, directQuotes: 0, directContracts: 0 };
    if (kind === "lead") { current.leadQuotes += seller.quotes; current.leadContracts += seller.contracts; }
  ...
  return [...grouped.values()].sort((a, b) => (b.leadContracts + b.directContracts) - (a.leadContracts + a.directContracts) || (b.leadQuotes + b.directQuotes) - (a.leadQuotes + a.directQuotes));
}
...
function ChannelsView({ payload }: { payload: DashboardPayload }) {
  ...
  return <><header className="page-head"><div><p className="eyebrow">Acquisizione e conversione</p><h1>Canali</h1><span>Totali, dettaglio venditori e coorti mensili o settimanali</span></div><div className="period-tabs"><button className={view === "monthly" ? "active" : ""} onClick={() => setView("monthly")}>Mensile</button><button className={view === "weekly" ? "active" : ""} onClick={() => setView("weekly")}>Settimanale</button></div></header>
    ...
    {analysis ? <><div className="metrics-grid channel-metrics"><Metric label="Lead in ingresso" value={leadTotals.entries} primary={percentage(leadTotals.appointments, leadTotals.entries)} secondary="lead → appuntamento" /><Metric label="Preventivi fisici" value={directTotals.quotes} primary={`${directRows.length} fonti`} secondary="walk-in e fonti dirette" /><Metric label="Preventivi totali" value={quotes} primary={percentage(contracts, quotes)} secondary="preventivi → contratti" /><Metric label="Contratti totali" value={contracts} primary={`${leadTotals.contracts} lead · ${directTotals.contracts} fisici`} secondary="dal file Venduto" /><Metric label="Show rate generale" value={percentage(show.presented, show.presented + show.noShows)} primary={`${show.presented} SI · ${show.noShows} NO`} secondary={`${show.pending} da aggiornare`} /></div>
      ...
      <section className="panel table-panel direct-channel-table"><header><div><h3>Conversione degli ingressi fisici</h3><p>Totale completo del periodo selezionato</p></div><FileText size={19} /></header><div className="data-table"><div className="data-row data-head"><span>Fonte</span><span>Preventivi</span><span>Quota sul totale</span><span>Contratti</span><span>Conversione</span></div>{directRows.map((item) => <div className="data-row" key={item.channel}><b>{item.channel}</b><strong>{item.quotes}</strong><span>{percentage(item.quotes, directTotals.quotes)}</span><strong>{item.contracts}</strong><span>{percentage(item.contracts, item.quotes)}</span></div>)}<div className="data-row data-total"><b>TOTALE FISICI</b><strong>{directTotals.quotes}</strong><strong>100%</strong><strong>{directTotals.contracts}</strong><strong>{percentage(directTotals.contracts, directTotals.quotes)}</strong></div></div></section>
      <section className="panel table-panel channel-seller-table"><header><div><h3>Divisione per venditore</h3><p>Preventivi e contratti distinti tra Lead e Fisici</p></div><Users size={19} /></header><div className="data-table"><div className="data-row data-head"><span>Venditore</span><span>Prev. lead</span><span>Contr. lead</span><span>CR lead</span><span>Prev. fisici</span><span>Contr. fisici</span><span>CR fisici</span><span>Prev. totali</span><span>Contr. totali</span></div>{sellerRows.map((item) => <div className="data-row" key={item.name}><b>{item.name}</b><span>{item.leadQuotes}</span><strong>{item.leadContracts}</strong><span>{percentage(item.leadContracts, item.leadQuotes)}</span><span>{item.directQuotes}</span><strong>{item.directContracts}</strong><span>{percentage(item.directContracts, item.directQuotes)}</span><strong>{item.leadQuotes + item.directQuotes}</strong><strong>{item.leadContracts + item.directContracts}</strong></div>)}</div></section>
      <div className="notice"><CircleAlert size={17} /><span><b>Venditori:</b> la divisione usa il Funzionario del preventivo e il Venditore del file Venduto. Il foglio dei lead non contiene il venditore, quindi il volume Lead e gli appuntamenti rimangono totali per evitare attribuzioni artificiali. I canali digitali restano sempre nel funnel Lead.</span></div></> : <section className="placeholder compact"><CircleAlert size={30} /><h2>Analisi canali in attesa</h2><p>Aggiorna Apps Script con la versione Canali.</p></section>}
  </>;
}
